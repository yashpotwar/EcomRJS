...
const ProductList = () => {
  ...
  const handleDelete = async (productId) => {
    try {
      await axios.delete(`http://192.168.29.71:5000/api/products/${productId}`);
  ...
  const handleEditSubmit = async () => {
    try {
      await axios.put(`http://192.168.29.71:5000/api/products/${editProduct.ID}`, editProduct);
            ...
            return (
              <div key={p.ID} className="product-card modern-card">
                <div className="product-thumb">
                  {p.ImagePath ? (
                    <img
                      src={`http://192.168.29.71:5000/uploads/${p.ImagePath}`}
                      alt={p.Name}
