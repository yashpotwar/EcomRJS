...
const ReviewApprovalPage = () => {
  ...
  const fetchPendingReviews = async () => {
    try {
      const res = await axios.get('http://192.168.29.71:5000/api/admin/reviews/pending');
      setReviews(res.data);
    } catch (error) {
  ...
  const handleApprove = async (id) => {
    try {
      await axios.put(`http://192.168.29.71:5000/api/admin/reviews/approve/${id}`);
  ...
  const handleReject = async (id) => {
    try {
      await axios.put(`http://192.168.29.71:5000/api/admin/reviews/reject/${id}`);
      setReviews(prev => prev.filter(r => r.ID !== id));
    } catch (err) {
